...
const Contact = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar />
      <main className="flex-grow flex flex-col items-center justify-center relative bg-background">
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-xl">
          <Dock className="items-end pb-3 glass-card border-neon-cyan hover:border-neon-purple transition-colors duration-300">
            {socialLinks.map((item, idx) => (
              <a key={idx} href={item.href} target="_blank" rel="noopener noreferrer">
                <DockItem className="aspect-square rounded-full bg-background/50 backdrop-blur-sm border border-white/10 hover:border-neon-cyan/50 transition-colors duration-300">
